...
const BookingPage = () => {
  ...
  const getRefundStatusColor = (refundStatus) => {
    if (!refundStatus) return "#666";
    const lowerStatus = refundStatus.toLowerCase();
    if (lowerStatus === "pending") return "#FFC107";
    if (lowerStatus === "approved") return "#4CAF50";
    if (lowerStatus === "rejected") return "#F44336";
    if (lowerStatus === "none") return "#666";
    return "#666";
  };
  ...
  const handleCancelBooking = async () => {
    ...
    // Check if it's a 25% payment (advance) or full payment
    const isAdvancePayment =
      selectedBooking.PaidAmount === selectedBooking.totalAmount * 0.25;

    // Validate UPI ID for full payment cases
    if (
      !isAdvancePayment &&
        ...
        alert(
          `Booking cancelled successfully. ${
            isAdvancePayment
              ...
              : `Refund of ₹${response.data.refundAmount} is pending approval.`
          }`
        );
        closeCancelModal();
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const SP = await axios.get(
        `${BASE_URL}/api/admin/all/admins/data`
      );
  ...
  return (
    <div
      style={{
        ...
        minHeight: "100vh",
        background:
          "linear-gradient(135deg, rgba(255,228,225,0.9) 0%, rgba(240,248,255,0.9) 50%, rgba(220,240,255,0.9) 100%)",
    ...
    >
      ...
      <div className="table-container">
        <table>
          <thead>
            <tr style={{ background: "#d9f5ef", color: "black" }}>
