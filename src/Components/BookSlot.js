...
const BookSlot = () => {
  ...
  const fetchManPower = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/api/admin/get-manpower/${encodeURIComponent(
          parlor.email
        )}`
      );
      ...
      const filteredBookings = response.data.bookings
        .filter(
          (booking) =>
            ...
            booking.date && // Ensure date exists
            typeof booking.date === "string" && // Ensure date is a string
  ...
  const handleRelatedServiceChange = (e) => {
    ...
    if (checked) {
      // Calculate new duration if the service is added
      ...
      // Check if the new duration causes a conflict
  ...
  const handleToggleModal = (category) => {
    ...
    // Clear error when closing the modal
    setErrors((prev) => ({ ...prev, relatedServices: "" }));
  };
          ...
          >
            ...
            >
              ...
              <IconButton sx={{ padding: "0.25rem" }}>
                {openModals[category] ? (
                  <ExpandLess sx={{ color: "#1abc9c", fontSize: "1rem" }} />
                ) : (
                  <ExpandMore sx={{ color: "#1abc9c", fontSize: "1rem" }} />
                )}
              </IconButton>
            </Box>
            <Modal
  ...
  return (
    <div className="container my-5">
      <style>
        {`
          .book-slot-container {
            background: linear-gradient(135deg, #f9fbfc 0%, #e9eff2 100%);
            border-radius: 14px;
            padding: 1rem;
            ...
            <Box className="shop-info-card text-center">
              {Object.keys(parlor).length === 0 ? (
                ...
                <>
                  ...
                  <Typography
                    sx={{ color: "#666666", mb: 1, fontSize: "0.85rem" }}
                  >
                    {parlor.style || "Style"}
                  </Typography>
                  <Typography
          ...
          <div className="col-12 col-md-8">
            <Box className="booking-form-card">
              ...
              <Button
                ...
                sx={{
                  mt: 2,
                  backgroundColor: "rgba(20, 209, 171)",
