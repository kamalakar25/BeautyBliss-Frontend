...
const ServicePage = () => {
  ...
  const handleAddService = async () => {
    const email = localStorage.getItem('email');
    try {
      const response = await axios.get(`${BASE_URL}/api/admin/check-employees/${email}`); // <-- update your actual API URL
      const { hasEmployees } = response.data;
  
      if (hasEmployees) {
        // If employees exist, proceed normally
        setFormData({ serviceName: '', style: '', price: '', imageFile: null, imagePreview: '' });
        setShowForm(true);
        setIsEditing(false);
      } else {
        // If no employees, alert the user
